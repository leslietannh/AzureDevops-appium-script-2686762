...
public class TestBase {
    ...
    public void setup(DesiredCapabilities desiredCaps, double retinaScale) throws Exception {
        ...
        this.isIos = MobilePlatform.IOS.equals(desiredCaps.getCapability(MobileCapabilityType.PLATFORM_NAME));
    ...
    public String switchToWebContext() throws Exception {
        ...
        for (int tryTime = 1; tryTime <= 3; tryTime++) {
            System.out.println(String.format("Find a web context, %s time", convertToOrdinal(tryTime)));
    ...
    public MobileElement findElementBy(By locator) {
        System.out.println(String.format("Find element by: %s", locator));

        MobileElement element = driver.findElement(locator);
        if (!element.isDisplayed()) {
            WebDriverWait wait = new WebDriverWait(driver, Config.IMPLICIT_WAIT_IN_SECOND);
    ...
    public void sendKeysToActiveElement(String keys) throws Exception {
        ...
        Response response = httpClient.newCall(sendKeysBuilder.build()).execute();
    ...
    public void touchOnScrollableElement(By locator, String commandId) throws Exception {
        ...
        if (targetElementXpath == null) {
            throw new Exception("Cannot find element to touch");
        }

        MobileElement element = findElementBy(By.xpath(targetElementXpath.replace(IOS_XPATH_REDUNDANT_PREFIX, "")));
        hideKeyboard();

        setCurrentCommandId(currentCommandId);
        touchAtRelativePointOfElement(element, Double.parseDouble(infoMap.get("touchedElementRelativeX")), Double.parseDouble(infoMap.get("touchedElementRelativeY")));
    }
    ...
    public Point getAbsolutePoint(double relativePointX, double relativePointY, Rectangle rect) {
        double x = rect.x + rect.width * relativePointX;
        double y = rect.y + rect.height * relativePointY;
    ...
    public Device getAvailableDevice(String deviceName, String platformVersion) throws Exception {
        ...
        Response response = httpClient.newCall(deviceListBuilder.build()).execute();
        if (response.code() != 200) {
            throw new Exception(response.body().string());
        }
    ...
    public Device findOnlineDevice(DesiredCapabilities capabilities) throws Exception {
        ...
        while (tryTime <= Config.DEVICE_WAITING_MAX_TRY_TIMES) {
            System.out.println(String.format("Find online device: %s - %s - %s time", deviceName, platformVersion, convertToOrdinal(tryTime)));
            device = getAvailableDevice(deviceName, platformVersion);
            if (device != null) {
                System.out.println(String.format("Device found: %s - %s", deviceName, platformVersion));
                break;
            }
            tryTime++;
    ...
    public String convertToOrdinal(int i) {
